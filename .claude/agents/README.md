# Your team

> **@ summons an agent — a who. / runs a skill — a what.**

Type `@` in Claude Code and your team appears. Each agent is one file in this folder:
its job, its limits, its schedule, its report step. You are the manager — you can read
any job description, ask for a promotion, or fire someone by deleting their file.

**No one hired yet.** Say **"I want to hire an agent"** and the interview starts —
five questions, then your first team member is working the same session.

## The team

| Agent | Job | Shift | Status | Skill |
|---|---|---|---|---|

## What the team knows

| Skill | The judgment it applies | Used by |
|---|---|---|

**Status legend** — `Probation`: hired, everything stages, not yet proven to fire on
its own. `Hired`: its shift has fired at least once with nobody asking (an `auto:`
line in `status/shift-log.md` — `auto-test:` lines are the hire session's own wiring
test and never count). `Probation — not yet scheduled`: honest stopgap, runs only
when a session is open; fix the schedule before anything else.

How hiring works, in full: [`library/HIRING.md`](../../library/HIRING.md).
