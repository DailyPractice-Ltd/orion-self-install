...
| `packages.<slug>` | one entry per installed capability — Library packages **and** bespoke hires or taught skills (`kind: agent\|skill`, semver from `0.1.0`; `contracts/agent-anatomy.md`): `kind`, `version`, `installed_at`, `smoke_test_passed` | the moment that capability's smoke test genuinely passes — same no-optimism rule as the checklist |
